import {
    ...
    NamedProfileAlreadyExistsError,
...
describe('NamedProfileService', () => {
    ...
    test('getNamedProfilesMap', () => {
        ...
        namedProfileService.getNamedProfiles = () => [{id: '1', name: 'profile1'}, {id: '2', name: 'profile2'}]

        const namedProfilesMap = namedProfileService.getNamedProfilesMap()

        expect(namedProfilesMap).toEqual(new Map([
            ['1', {id: '1', name: 'profile1'}], ['2', {id: '2', name: 'profile2'}]
        ]))
    })
    ...
    test('editNamedProfile', async () => {
        ...
        expect(repository.updateProfile).toHaveBeenCalledWith('profileId', 'validName')
    ...
    test('validateNewProfileName', () => {
        ...
        expect(namedProfileService.validateNewProfileName('   profile   ')).toBe('profile')

    })

    test('validateNewProfileName, empty name', () => {
        const namedProfileService = new NamedProfilesService(null, null, null)
        expect(() => namedProfileService.validateNewProfileName('  ')).toThrow(new EmptyProfileNameError())
    })

    test('validateNewProfileName, default name', () => {
        const namedProfileService = new NamedProfilesService(null, null, null)
        expect(() => namedProfileService.validateNewProfileName(constants.defaultAwsProfileName))
            .toThrow(new DefaultIsNotAValidProfileNameError())
    })
    ...
    test('validateNewProfileName, existent name', () => {
        ...
        expect(() => namedProfileService.validateNewProfileName('profile'))
            .toThrow(new NamedProfileAlreadyExistsError())
    })
})
