...
*/
export var ToastLevel;
(function (ToastLevel) {
    ToastLevel[ToastLevel["info"] = 0] = "info";
    ToastLevel[ToastLevel["warn"] = 1] = "warn";
    ToastLevel[ToastLevel["error"] = 2] = "error";
    ToastLevel[ToastLevel["success"] = 3] = "success";
})(ToastLevel || (ToastLevel = {}));
let AppService = class AppService {
    constructor(modalService, electronService, toastr) {
        this.modalService = modalService;
        this.electronService = electronService;
        this.toastr = toastr;
        this.profileOpen = new EventEmitter();
        this.stsEndpointsPerRegion = new Map([
     ...
     */
    newWindow(url, show, title, x, y, javascript) {
        const opts = {
            width: 514,
            height: 550,
            resizable: true,
    ...
    logout() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                yield this.electronService.session.defaultSession.clearStorageData();
                // Clean localStorage
                localStorage.clear();
     ...
     * @param callback - the callback for the ok button to launch
     */
    inputDialog(title, placeholder, message, callback) {
        for (let i = 1; i <= this.modalService.getModalsCount(); i++) {
            this.modalService.hide(i);
        }
        this.getCurrentWindow().show();
    ...
    stsOptions(session) {
        ...
        if (session.region) {
            options = Object.assign(Object.assign({}, options), { endpoint: this.stsEndpointsPerRegion.get(session.region), region: session.region });
        }
        return options;
    }
    getUrl() {
