...
export class AwsAuthenticationService implements IAwsAuthenticationService {
  ...
  public async needAuthentication(idpUrl: string): Promise<boolean> {
    return new Promise((resolve) => {
      ...
      let idpWindow = this.windowService.newWindow(
        idpUrl, false, '', activeWindowPosition[0] + nearX, activeWindowPosition[1] + nearY);
      ...
      idpWindow.webContents.session.webRequest.onBeforeRequest(filter, (details, callback) => {
        ...
        if (details.url.indexOf('https://login.microsoftonline.com') !== -1 &&
          details.url.indexOf('/oauth2/authorize') !== -1) {
          idpWindow = null;
          resolve(true);
        }
  ...
  public async awsSignIn(idpUrl: string, needToAuthenticate: boolean): Promise<any> {
    ...
    let idpWindow = this.windowService.newWindow(
      idpUrl, needToAuthenticate, 'IDP - Login', activeWindowPosition[0] + nearX, activeWindowPosition[1] + nearY);
