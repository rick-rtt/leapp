...
export class IntegrationBarComponent implements OnInit, OnDestroy {
  ...
  public ngOnInit(): void {
    this.subscription = integrationsFilter.subscribe(() => {
      ...
      this.selectedIntegrations = this.awsSsoConfigurations.map((awsIntegration) =>
        ({ id: awsIntegration.id, selected: false }));
    });
  ...
  public applyContextMenu(index: number, awsSsoConfiguration: AwsSsoIntegration, event: any): void {
  ...
  public async forceSync(integrationId: string): Promise<void> {
  ...
  public gotoForm(modifying: number, awsSsoIntegration: AwsSsoIntegration): void {
  ...
  public delete(awsSsoIntegration: AwsSsoIntegration): void {
    // Ask for deletion
    // eslint-disable-next-line max-len
    this.windowService.confirmDialog(`Deleting this configuration will also logout from its sessions: do you want to proceed?`, async (res) => {
