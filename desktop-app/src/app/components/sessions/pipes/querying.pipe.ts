...
export class QueryingPipe implements PipeTransform {
  public transform(sessions: Session[], query: string, workspace: Workspace): Session[] {
    if (query !== '') {
      return sessions.filter((s) => {
        const idpID = workspace.idpUrls.filter(
          (idp) => idp && idp.url.toLowerCase().indexOf(query.toLowerCase()) > -1
        ).map((m) => m.id);

        const integrationID = workspace.awsSsoIntegrations.filter(
          (integration) => integration.alias.toLowerCase().indexOf(query.toLowerCase()) > -1
        ).map((m) => m.id);
