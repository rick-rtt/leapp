...
describe('CliAwsAuthenticationService', () => {
  test('needAuthentication', async () => {
    ...
    cliAwsAuthenticationService.getNavigationPage = async (headlessMode: boolean) => {
      ...
      return of(page as unknown as puppeteer.Page).toPromise()
    }
    cliAwsAuthenticationService.isRequestToIntercept = () => {
      return true
    }
  ...
  class PageStub {
    ...
    private callback: Map<string,(request: puppeteer.HTTPRequest) => Promise<void>>
    ...
    constructor(public expectedIdpUrl: string, public requestStub: any) {
      ...
      this.callback = new Map<string, (request: puppeteer.HTTPRequest) => Promise<void>>()
    }
